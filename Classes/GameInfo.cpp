...
void GameInfo::initData()
{
	...
	for(int i = 0; i < size; i++)
	{
		...
		for(iter = vData[i].begin(); iter != vData[i].end(); iter++) 
		{
			...
			if (iter->first == "resource_path")
				stHero.sResourcePath = iter->second;
			...
			if (iter->first == "skill_rc_path")
				stHero.stSkill.sResourcePath = iter->second;
			CCLOG("key: %s, value: %s", iter->first.c_str(), iter->second.c_str());
		}
	...
	for(int i = 0; i < size; i++)
	{
		...
		for(iter = vData[i].begin(); iter != vData[i].end(); iter++) 
		{
			...
			if (iter->first == "resource_path")
				stItem.sResourcePath = iter->second;
			if (iter->first == "value")
	...
	for(int i = 0; i < size; i++)
	{
		...
		for(iter = vData[i].begin(); iter != vData[i].end(); iter++) 
		{
			...
			if (iter->first == "resource_path")
				stMonster.sResourcePath = iter->second;
			...
			if (iter->first == "skill_rc_path")
				stMonster.stSkill.sResourcePath = iter->second;
			CCLOG("key: %s, value: %s", iter->first.c_str(), iter->second.c_str());
		}
